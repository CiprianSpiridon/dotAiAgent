...
describe('CLI server script resolution', () => {
  test('finds server.ts via repo root (3 levels up from binary)', () => {
    const root = fs.mkdtempSync('/tmp/gstack-cli-');
    const execPath = path.join(root, '.claude/skills/browse/browse');
    const serverPath = path.join(root, 'src/browse/src/server.ts');
  ...
  test('finds server.ts via symlink resolution', () => {
    const repoDir = fs.mkdtempSync('/tmp/gstack-repo-');
    const projectDir = fs.mkdtempSync('/tmp/gstack-project-');

    // Real binary and server in repo
    const realBinDir = path.join(repoDir, '.claude/skills/browse');
    const serverPath = path.join(repoDir, 'src/browse/src/server.ts');
    fs.mkdirSync(realBinDir, { recursive: true });
    fs.mkdirSync(path.dirname(serverPath), { recursive: true });
    fs.writeFileSync(path.join(realBinDir, 'browse'), '');
    fs.writeFileSync(serverPath, '// test server\n');

    // Symlink .claude in project -> repo .claude
    const projectClaude = path.join(projectDir, '.claude');
    fs.symlinkSync(path.join(repoDir, '.claude'), projectClaude);

    // The binary path goes through the symlink
    const symlinkBinPath = path.join(projectDir, '.claude/skills/browse/browse');
    ...
    // realpathSync resolves /tmp -> /private/tmp on macOS, so compare real paths
    expect(fs.realpathSync(resolved)).toBe(fs.realpathSync(serverPath));
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(projectDir, { recursive: true, force: true });
  });

  test('finds server.ts via ~/.dotAiAgent fallback', () => {
    const fakeHome = fs.mkdtempSync('/tmp/gstack-home-');
    const serverPath = path.join(fakeHome, '.dotAiAgent/src/browse/src/server.ts');
    fs.mkdirSync(path.dirname(serverPath), { recursive: true });
    fs.writeFileSync(serverPath, '// test server\n');

    // Binary at a location with no repo structure around it
    const isolatedBin = path.join(fs.mkdtempSync('/tmp/gstack-isolated-'), 'browse');
