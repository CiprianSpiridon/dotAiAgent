...
const LOCK_STALE_MS = 15000; // Consider lock stale after 15s
...
export function resolveServerScript(
  ...
  // Compiled binary: binary is at .claude/skills/browse/browse,
  // server.ts is at src/browse/src/server.ts (3 levels up to repo root)
  if (execPath) {
    const binDir = path.dirname(execPath);
    const repoRoot = path.resolve(binDir, '..', '..', '..');
    const adjacent = path.join(repoRoot, 'src', 'browse', 'src', 'server.ts');
    if (fs.existsSync(adjacent)) {
      return adjacent;
    }
  }

  // Symlink install: .claude/ is a symlink to e.g. ~/.dotAiAgent/.claude/
  // Resolve the symlink to find the source repo
  if (execPath) {
    try {
      const realExecPath = fs.realpathSync(execPath);
      const realBinDir = path.dirname(realExecPath);
      const realRepoRoot = path.resolve(realBinDir, '..', '..', '..');
      const symlinked = path.join(realRepoRoot, 'src', 'browse', 'src', 'server.ts');
      if (fs.existsSync(symlinked)) {
        return symlinked;
      }
    } catch {}
  }

  // Common clone location: ~/.dotAiAgent
  {
    const home = env.HOME || env.USERPROFILE || '';
    if (home) {
      const common = path.join(home, '.dotAiAgent', 'src', 'browse', 'src', 'server.ts');
  ...
  // Fallback: server.ts next to the binary (flat install)
  if (execPath) {
    const flat = path.resolve(path.dirname(execPath), 'server.ts');
