...
const NETWORK_SETTLE_MS = 5000; // Wait up to 5s for response before flushing
...
function flushBuffers(final = false) {
  ...
  if (newNetworkCount > 0) {
    ...
    for (let i = 0; i < newEntries.length; i++) {
      ...
      if (final || e.status !== undefined || (now - e.timestamp > NETWORK_SETTLE_MS)) {
        prefixLen = i + 1;
      } else {
...
async function findPort(): Promise<number> {
  ...
  // Fallback: scan range
  const start = parseInt(process.env.BROWSE_PORT_START || '9999', 10);
